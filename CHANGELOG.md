...
Grace CLI 4.0.5 makes linting faster, more robust, and less noisy across ecosystems, and this fork folds in a round of adapter and parser work on top of it. The default ignored-directory list grows to 64 well-known tooling, build-output, and test-report names (covering Playwright, Allure, Newman, Cucumber, Stryker, Python, and JavaScript/TypeScript tooling), so grace lint automatically skips generated artifacts without extra configuration. Repeated lint runs and CI re-runs are now faster thanks to a content-addressed cache of per-file language analyses that lives outside the project, never touches governed .grace state, and invalidates automatically when files change. Unreadable directories are now skipped with a warning instead of aborting the whole command, and runtime log-marker evidence is credited more broadly: the base pattern now recognizes Go, Java, and C# loggers alongside JavaScript, and is further extended with facade-aware matching for Android's Log.* calls and Dart's debugPrint/developer.log. The optional .grace-lint.json configuration file is now documented for projects that need custom ignore rules. On top of that, the fork adds in-process heuristic language adapters for Kotlin (.kt, .kts), Swift (.swift), and Vue (.vue), and rewrites the Dart adapter onto the same in-process path with no external analyzer subprocess; MODULE_MAP mismatches surfaced by these heuristic adapters are reported as the analysis.heuristic-map-mismatch warning rather than the markup.module-map-mismatch error used for exact adapters. It also carries a series of point fixes to file-local markup parsing and log-marker recognition: multi-line MODULE_MAP entries joined by paren balance, quote-parity corruption contained to a single logical line, unknown CLI arguments rejected instead of silently accepted, wrapped markup lists parsed as one item instead of one per line, Log-facade calls recognized for health.required-log-marker-not-found, subtree-aware command matching with a deterministic POSIX shell for assertions, one analysis.heuristic-confidence emission per adapter per run, phantom Function exports removed from function-typed declarations, symbols correctly extracted from name(args) MODULE_MAP entries, Dart raw strings parsed without escape handling in the depth scanner, and bare-word map continuations no longer misread as symbols.
...
* feat: port fork language adapters onto grace 4 ([7646157](https://github.com/osovv/grace-marketplace/commit/7646157))
* chore: mark grace entrypoint executable for local link installs ([3cc402c](https://github.com/osovv/grace-marketplace/commit/3cc402c))
* fix: document internal surface, not just public exports ([647c86d](https://github.com/osovv/grace-marketplace/commit/647c86d))
* fix: join wrapped lines by paren balance before marker check ([ab85043](https://github.com/osovv/grace-marketplace/commit/ab85043))
* fix: contain quote-parity corruption to one logical line ([b24970f](https://github.com/osovv/grace-marketplace/commit/b24970f))
* fix: reject unknown arguments instead of accepting them silently ([4253cdb](https://github.com/osovv/grace-marketplace/commit/4253cdb))
* fix: parse a wrapped markup list as one item, not one item per line ([9d87957](https://github.com/osovv/grace-marketplace/commit/9d87957))
* fix: recognize Log-facade calls in health.required-log-marker-not-found ([abe9e4d](https://github.com/osovv/grace-marketplace/commit/abe9e4d))
* fix: recognize debugPrint and developer.log in health.required-log-marker-not-found ([8a25e4b](https://github.com/osovv/grace-marketplace/commit/8a25e4b))
* fix: subtree-aware command matching and deterministic POSIX shell for assertions ([8f5381e](https://github.com/osovv/grace-marketplace/commit/8f5381e))
* fix(lint): emit analysis.heuristic-confidence once per adapter per run ([c0d71f8](https://github.com/osovv/grace-marketplace/commit/c0d71f8))
* fix(lint): stop exporting phantom Function from function-typed declarations ([147e569](https://github.com/osovv/grace-marketplace/commit/147e569))
* fix(lint): extract symbol from name(args) MODULE_MAP entries ([cb1aa6c](https://github.com/osovv/grace-marketplace/commit/cb1aa6c))
* fix(lint): parse Dart raw strings without escape handling in depth scanner ([60fa768](https://github.com/osovv/grace-marketplace/commit/60fa768))
* fix(lint): stop treating bare-word map continuations as symbols ([9f58d8c](https://github.com/osovv/grace-marketplace/commit/9f58d8c))
