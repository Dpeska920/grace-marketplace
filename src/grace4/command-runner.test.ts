...
describe.skipIf(process.platform === "win32")("runDeclaredCommands shell contract", () => {
  test("executes commands through POSIX sh regardless of the caller's $SHELL", async () => {
    const { options } = fixture();
    // Word splitting: sh yields n=3; zsh without SH_WORD_SPLIT would yield 1.
    // The command must pass on any machine, including ones where $SHELL=zsh.
    const command = 'tgt=$(printf "a\\nb\\nc\\n"); n=0; for t in $tgt; do n=$((n+1)); done; test "$n" -eq 3';
    const previousShell = process.env.SHELL;
    process.env.SHELL = "/bin/zsh";
    try {
      const summary = await runDeclaredCommands(declared([command]), options);
      expect(summary.status).toBe("passed");
...
describe.skipIf(process.platform === "win32")("runDeclaredCommands timeout and process groups", () => {
  ...
  // Background-child group kill relies on setsid to make the shell its own
  // process-group leader; without it (e.g. stock macOS, no util-linux) the
  // spawned shell shares the test runner's group and this assertion cannot
  // hold regardless of command-runner correctness.
  test.skipIf(!Bun.which("setsid"))("timeout kills the whole process group including background children", async () => {
