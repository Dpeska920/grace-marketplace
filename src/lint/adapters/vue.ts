// Language adapter for Vue SFC (.vue) files.
// Extracts exports from <script>/<script setup> block via the TypeScript adapter;
// heuristic confidence. For <script setup>: extracts top-level bindings
// (auto-exposed) + defineExpose() keys.
//
// Ported from the fork/v3 adapter. v3 called an exported `analyzeTypeScriptSource`
// helper from typescript.ts; v4's typescript.ts only exports the adapter factory,
// so this adapter drives the same analysis through `createTypeScriptAdapter().analyze()`
// on a virtual `.ts` path — equivalent behavior, no change to typescript.ts required.
...
const tsAdapter = createTypeScriptAdapter();
 ...
 * We also honor explicit `export` declarations via the TypeScript adapter's own analysis.
...
function mergeAnalyses(
  ...
  return {
    ...
    localSymbols: new Set([...base.localSymbols, ...next.localSymbols]),
...
export function createVueAdapter(): LanguageAdapter {
  return {
    ...
    analyze(filePath, text) {
      ...
      let merged: LanguageAnalysis = emptyAnalysis();
      for (const block of blocks) {
        const inner = tsAdapter.analyze(virtualPath, block.content);
