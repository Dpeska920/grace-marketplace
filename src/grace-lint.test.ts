...
describe("lintGraceProject", () => {
  ...
  it("reports analysis.heuristic-confidence once per adapter per lint run", async () => {
...
`;
    ...
    const lintResult = await lintGraceProject(root);
    const heuristicIssues = lintResult.issues.filter((issue) => issue.code === "analysis.heuristic-confidence");
